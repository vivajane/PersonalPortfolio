...
const Header = () => {
  ...
  return (
    <div
      className={`w-full z-50 bg-[#faf9f8] fixed ${
        show && "overflow-hidden h-screen "
      }`}
    >
      <div className=" flex justify-between w-full items-center py-6 sm:px-12 px-8 md:px-24">
        <h1 className="md:text-xl w-1/2 font-comfortaa font-bold">
          KosiVivian
        </h1>
        ...
        <ul
          onClick={onClose}
          className={` md:hidden px-24 space-y-6 flex flex-col items-center justify-evenly gap-10 absolute top-[73px] left-0 w-full h-screen transition translate-x-30 z-10 duration-500 bg-[#FDC435]  ${
            show ? "block" : "hidden"
          }`}
        >
          {navlinks.map((item) => (
